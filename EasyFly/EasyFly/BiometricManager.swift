...
final class BiometricManager {
    ...
    func authenticateWithBiometrics(for email: String, reason: String = "Autenticar para acessar sua conta") async -> Bool {
        ...
        guard isBiometricLoginEnabled(for: e) else {
            os_log("Biometric not enabled: %{private}@", log: logger, type: .warning, e)
            return false
        }
        ...
        return await withCheckedContinuation { cont in
            ctx.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { success, err in
                if success {
                    ...
                    os_log("Biometric auth failed: %{private}@", log: logger, type: .warning, e)
    ...
    func verifyBiometricSecret(for email: String) -> Bool {
        ...
        guard let encrypted = defaults.string(forKey: secretKey) else {
            os_log("No biometric secret found: %{private}@", log: logger, type: .warning, e)
            return false
        }
